...
| [04. Release v0.4.0.md](releases/04.%20Release%20v0.4.0.md) | Phase 4, cut from `f087314` on 2026-08-28. Pre-flight and the release build complete: 23 migrations against an empty database, the N−1 rollback to `v0.3.0` **passed first time**, and the phase exit demo driven through all eight browser flows against the release images. **The tag is blocked** — the working token cannot merge PR #219, so the release commit is not on `main`. Two findings against the deployment path, both filed here |
