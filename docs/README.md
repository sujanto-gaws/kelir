...
| Decision records | **35 adopted, one superseded** (0009 by 0010), **one `Draft`** ([0037](architectures/adr/0037.%20ERP%20Posting%20Runs%20Inside%20the%20Document%20Transaction.md), blocked by [#371](https://github.com/sujanto-gaws/kelir/issues/371)). **0001–0028** came from the 2026-09-01 retrofit pass over the architecture documents and the `D-n` table; every record since is written as its decision is taken. This line had read *28 adopted* since that pass — the count is now checked by `adr_records_are_current.rs` at the level that matters, which is the index agreeing with each record rather than the total |
