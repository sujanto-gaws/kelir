...
**Status:** Living · **Last updated:** 2026-09-26
...
| [06. Building an ERP on Kelir.md](architectures/06.%20Building%20an%20ERP%20on%20Kelir.md) | **Draft, nothing scheduled** — the layering an ERP over Kelir would take: what the platform provides, the ledger concepts it has none of, the posting seam, and the platform gaps an ERP forces. Scope is issue [#371](https://github.com/sujanto-gaws/kelir/issues/371) |
...
| [0036](architectures/adr/0036.%20The%20Hook%20Chain%20Ships%20Its%20Before%20Half%20First.md) | The lifecycle hook chain ships its before half, driven by a service task; after-hooks wait for the outbox | Adopted — the after half partially superseded by 0041 |
| [0037](architectures/adr/0037.%20ERP%20Posting%20Runs%20Inside%20the%20Document%20Transaction.md) | ERP posting runs inside the document's own transaction, not on the outbox | **Draft** — blocked by [#371](https://github.com/sujanto-gaws/kelir/issues/371) |
| [0038](architectures/adr/0038.%20Kelir%20Patterns%20Are%20the%20Linear-Time%20Subset.md) | A Kelir pattern is what the `regex` crate compiles, with `\d`, `\w` and `\s` written out | **Adopted** — `D-15`'s shape half, four sprints after the [operator-parity spike](../projects/spikes/01.%20JFSS%20Operator%20Parity.md) §2.7 measured it. `fancy-regex` is the rejected alternative, **on the NFR rather than the capability**: a pattern is written by a tenant's configuration author and stored, so backtracking makes a denial of service reachable from a form definition. Neither engine closes the silent half — ECMA-262's `\d` is ASCII and this crate's is Unicode — so the save is the only place it can be caught. The cost is stated: server-side password complexity becomes several rules rather than one |
| [0039](architectures/adr/0039.%20A%20Dashboard%20Widget%20Is%20a%20Purpose-Built%20Endpoint.md) | A dashboard widget is a purpose-built endpoint in `modules::reporting`; no FR-RPT row ships as a RAD list definition | **Adopted** — **D-78**'s shape half, taken at Phase 8 planning while `modules::reporting` was still one line. Four of Phase 8's five widgets have rows a RAD list cannot carry — two over `workflow_tasks`, one an aggregate — and the fifth, FR-RPT-003, genuinely is a page of documents, which is why **per-widget choice is the rejected alternative rather than a straw one**. It loses on the dashboard having two answers to *what may this viewer see*. The cost is stated: the dashboard is not configurable by a deployment, and every widget costs an endpoint |
| [0040](architectures/adr/0040.%20A%20Chart%20Is%20Drawn%20with%20Unovis,%20Off%20the%20First-Load%20Path.md) | A chart is drawn with Unovis (`@unovis/vue` + `@unovis/ts`, pinned at 1.7.0), reached only through a lazily loaded component; declaration files go unchecked | **Adopted** — FR-RPT-004's chart, the first in the tree. **The recommendation put to the product owner was option A — bars as Tailwind widths, no dependency — and the decision was Unovis**, the charting layer of the UI kit already in use; Chart.js was the third option. The chart chunk is 54.99 kB gzipped and fetched only when the card renders, first load grows by 1.0 kB, and `check-bundle-split.mjs` now holds both libraries to that. The cost is stated: every Unovis release tried ships declarations `vue-tsc` cannot compile, so `skipLibCheck` is on and **no `.d.ts` is type-checked, ours included** |
| [0041](architectures/adr/0041.%20Every%20Workflow%20Transition%20Writes%20an%20Outbox%20Event,%20and%20After-Hooks%20Are%20Its%20First%20Consumer.md) | Every committed workflow transition writes one `Workflow.Transitioned` event to `outbox_events` in its own transaction, and a worker in `modules::outbox` delivers it to the `after_workflow_transition` chain, resolved at delivery | **Adopted** 2026-09-24 — [#519](https://github.com/sujanto-gaws/kelir/issues/519), [#537](https://github.com/sujanto-gaws/kelir/pull/537), Sprint 20 row 3, **D-79**'s head of Phase 9. It lifts [ADR-0036](architectures/adr/0036.%20The%20Hook%20Chain%20Ships%20Its%20Before%20Half%20First.md)'s main negative — JWSS `actions` run — and ADR-0036 stays `Adopted` for the before half. **One event per transition, not a row per non-empty chain**, is the chosen shape, so a second consumer reads the same rows without touching the engine. Retries rerun the whole chain (30 s doubling, `DEAD_LETTER` on the sixth failure), and the circuit breaker is derived from `document_hook_executions` rather than stored. The cost is stated: a row per transition, most of them for empty chains; a handler sees the document at delivery, not at commit; and dead letters have no screen |
...
| Decision records | **38 adopted, one superseded** (0009 by 0010), **one `Draft`** ([0037](architectures/adr/0037.%20ERP%20Posting%20Runs%20Inside%20the%20Document%20Transaction.md), blocked by [#371](https://github.com/sujanto-gaws/kelir/issues/371)). **0001–0028** came from the 2026-09-01 retrofit pass over the architecture documents and the `D-n` table; every record since is written as its decision is taken. This line had read *28 adopted* since that pass — the count is now checked by `adr_records_are_current.rs` at the level that matters, which is the index agreeing with each record rather than the total |
| Concepts 01–04 | **Superseded** — background and intent only (authority rule 7) |
| Planning | Complete — sprint plan and product backlog cover all 164 FRs. Scope decisions **D-1…D-95** are recorded in [Product Backlog](../projects/planning/02.%20Product%20Backlog.md) §6. **D-1…D-94 are resolved**, the last two (**D-93**, **D-94**) on 2026-09-25, and **D-7** is superseded by **D-18**. **D-95 is open** since 2026-09-26: whether [#426](https://github.com/sujanto-gaws/kelir/issues/426)'s workflow builder is a canvas, and where a canvas keeps its layout. Phase 9 is Sprints 20–22 by **D-90**, with `v0.9.0` at Sprint 22 and `v1.0.0` after hardening at Sprint 24. `v0.9.0` is the integration core, not plugins, by **D-94**. Sprints 0–21 are detailed in [Sprint Plan](../projects/planning/01.%20Sprint%20Plan.md) §5, and later sprints are detailed at each phase boundary |
