...
> |---|---|---|
> | **Flags** | **None**: `params.flags` absent or empty. `validation.pattern` carries none | Every flag. `i` splits on `k` and `s`; `m`, `u`, `v`, `y` and unknown or repeated letters are in the tables above, and `s` changes only `.`, which is outside the subset anyway. `g` and `d` split nothing in the probe, and change nothing for a single `test` either |
...
> | **Groups** | `(…)`, `(?:…)`, and `\|` between alternatives. **A group with a quantifier after it has no `\|` and no quantifier anywhere inside it** (added in 1.5.3) | A quantified group holding either, such as `(?:[a-z]\|[a-z0-9])*` or `(a+)+`: write `[a-z0-9]*` or `a+`. Named groups, inline flags, look-around |
> | **Quantifiers** | `*`, `+`, `?`, `{n}`, `{n,}`, `{n,m}`, each optionally followed by `?` | A space inside the braces; two on one atom; `{,n}` |
>
> **Case-insensitive matching is written out** — `[Aa][Bb][Cc]`, not `abc` with `i`.
>
> **Agreement also depends on the browser finishing the match, and inside the subset it may not** (1.5.3, [#493](https://github.com/sujanto-gaws/kelir/issues/493)). The `regex` crate matches in time linear in the value, and that is [ADR-0038](../architectures/adr/0038.%20Kelir%20Patterns%20Are%20the%20Linear-Time%20Subset.md)'s reason for it. The browsers backtrack, and each handles a match that runs long in its own way. **V8 keeps going, and the tab is frozen until it answers.** **SpiderMonkey throws after about seven seconds**, and the renderer counts an exception as a violation (`validation.ts`, the `regex` rule's `catch`), so the form refuses the value. **JavaScriptCore can stop and answer *no match***. Measured 2026-09-17 in Playwright's builds, against the crate built as `compile_pattern` builds it. "Over 20 s" means the probe stopped waiting, not that the engine had given up:
>
...
>
> **Three things follow, and none of them is a guarantee:**
>
