...
>
> **Since 1.5.6 the renderer does not run a `regex` rule once a keyword has failed on the field** (#496). A value longer than `validation.maxLength` is then not matched by the rule in the browser. The field's verdict is unchanged, because the keyword's violation already outranked the rule's.
>
