...
use super::domain::{
    validate_assign_role, AssignRoleRequest, AssignedRole, EmploymentType, PartyProfiles,
...
pub async fn assign_role(
...
) -> Result<(bool, AssignedRole), AppError> {
    ...
    // The assignment as this call stated it, and only that.
    //
    ...
    //
    // Narrowing it to the row was not enough. `update_party_role` merges
    // `status`, `comments` and `attributes_json`, so a caller who sent none of
    // them still got back what a previous writer had put there — the same leak
    // one field over (#119). What the row holds is
    // `master-data:party-role:read`'s to give.
    //
    // Gating the extra members here would have closed it too, and is the second
    // gate #104 rejected: two rules to keep in step where one will do. An answer
    // built from the request cannot leak what the request did not carry, and it
    // needs no read-back — which is why `find_party_role_by_id` is gone with it.
    Ok((creating, AssignedRole::echo(role_type, &request)))
}
