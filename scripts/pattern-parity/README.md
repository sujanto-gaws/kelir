...
Each case gets a fresh page. A match running past `TIMEOUT` is reported as `TIMEOUT`, and the browser is relaunched, because a page stuck in a match cannot be interrupted. **An exception is reported as `UNDECIDED`**, as the renderer has treated one since [#496](https://github.com/sujanto-gaws/kelir/issues/496): the server decides that value on submit. The registry 1.5.3 table was produced before that, when the script and the renderer counted an exception as a violation, which is the table's *refused*. It produces the registry 1.5.3 table, run 2026-09-17 in Chromium 151, Firefox 153 and WebKit 26.5. Timings are one run on one machine: read them as orders of magnitude.
